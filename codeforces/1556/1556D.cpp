...
void solve()
{
	...
	cin >> n >> k;
	vi a(n), b;
	int x = askAND(1, 2)+askOR(1,2);
	int y = askAND(2, 3)+askOR(2,3);
	int z = askAND(3,1)+askOR(3,1);
	a[0] = (x+z-y)/2LL, a[1] = (x+y-z)/2LL, a[2] = (y+z-x)/2LL;
	FOR(i, 3, n){
		a[i] = askAND(1, i+1)+askOR(1, i+1)-a[0];
	}
	sort(all(a));
	finish(a[k-1]);
}
