...
void solve()
{
	...
	int sink = 0, source = 0;
	F0R(i,n){
		if (a[i]-k<0)SK[a[i]]++, sink++;
		else if (a[i]-k>0) SRC[a[i]]++, source++;
	}
	if (sz(SK)&&sz(SRC)==0){
		cout << 1;
		return;
	...
	if (sink<=1||source<=1){
		Mint ans = fact[n];
		for(auto [x,y]:SK) ans/=fact[y];
		for(auto [x,y]:SRC)ans/=fact[y];
		ans/=fact[n-sink-source];
		cout << ans << "\n";
	}else{
		Mint ans = 2*fact[source]*fact[sink];
		for(auto [x,y]:SK){
			ans/=fact[y];
		}
		for(auto [x,y]:SRC){
