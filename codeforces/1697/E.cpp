...
void solve()
{
	...
	vi minDist(n, INF);
	for (int i : rep(n)) cin >> x[i] >> y[i];
	for (int i : rep(n)) for (int j : rep(n)) {
		dist[i][j] = abs(x[i] - x[j]) + abs(y[i] - y[j]);
		if (i != j) ckmin(minDist[i], dist[i][j]);
	}
	vector<bool> good(n);
	vi setSZ;
	for (int i : rep(n)) {
		...
		auto dfs = y_combinator([&](auto self, int u) -> void {
			vis[u] = 1;
			comp.pb(u);
			for (int v : rep(n)) if (!vis[v] && dist[u][v] == k) self(v);
		});
		dfs(i);
		bool ok = 1;
		for (auto &ch : comp) for (auto &e : comp) {
			if (e == ch) continue;
			if (minDist[ch] != k || minDist[e] != k || dist[ch][e] != k) {
				ok = 0;
				break;
			}
		}
		if (ok == 0) {
			...
			setSZ.pb(sz(comp));
			for (auto &e : comp) good[e] = 1;
		}
	}
	Mint ans = 0;
	vector<vector<Mint>> dp(sz(setSZ) + 1, vector<Mint>(n + 1));
	dp[0][0] = 1;
	for (int i : rep(1, sz(setSZ) + 1)) {
		// single
		for (int j : rep(1, n + 1)) {
			dp[i][j] += dp[i - 1][j - 1]; 
		}
		// individual
		if (setSZ[i - 1] == 1) continue;
		for (int j : rep(setSZ[i - 1], n + 1)) {
			dp[i][j] += dp[i - 1][j - setSZ[i - 1]];
		}
	}
	dbg(dp);
	for (int k : rep(n + 1)) {
		ans += dp[sz(setSZ)][k] * C.binom(n, k) * C.fact[k];
	}
	cout << ans << '\n';
}
