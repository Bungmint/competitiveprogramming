...
void solve()
{
	...
	while(sz(odd)){
		if (sz(even)){
			while(sz(odd)&&sz(even)){
				int x = *even.rbegin(); even.erase(even.find(x));
				int val = min(sz(odd)+1, x)/2;
				x -= 2*val;
				REP(val){
					if (sz(odd)==1){
						even.insert(*odd.begin()+1);
						odd.erase(odd.begin());
					}else{
						even.insert(*odd.begin()+1);
						odd.erase(odd.begin());
			...
			if (int x = *odd.rbegin(); x==1||sz(odd)==1){
				cout << -1 << "\n";
				return;
			}else {
