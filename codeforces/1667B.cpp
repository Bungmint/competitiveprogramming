...
void solve()
{
	int n;
	cin >> n;
	vl a(n + 1), pref(n + 1), ind, dp(n + 1, -LINF);
	FOR(i, 1, n + 1) cin >> a[i], pref[i] = pref[i - 1] + a[i], ind.pb(pref[i]);
	ind.pb(0);
	sort(all(ind)), ind.resize(unique(all(ind)) - ind.begin());
	SegTree<Max> s(sz(ind)), t(sz(ind));
	// s - diff, t - same
	int it = lb(all(ind), 0) - ind.begin();
	dp[0] = 0;
	s.upd(it, 0), t.upd(it, 0);
	dbg(ind);
	FOR(i, 1, n + 1) {
		pref[i] = lb(all(ind), pref[i]) - ind.begin();
		dbg(pref[i]);
		Max z = s.query(0, pref[i]);
		Max w = t.query(pref[i], pref[i] + 1);
		if (z.x > -LINF) {
			ckmax(dp[i], z.x + i);
		}
		if (w.x > -LINF) {
