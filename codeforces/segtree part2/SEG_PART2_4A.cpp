...
struct LazySegTree{
	...
	inline ll lazy_apply(ll lazy_val, ll new_val){
		if (new_val<=0||lazy_val==seg_lazy_sentinel) return new_val;
		if (lazy_val<=0) return lazy_val-new_val;
		return lazy_val+new_val;
	}
	inline ll lazy_func(ll cur_val, ll lazy_val, int l, int r){
		if (lazy_val<=0) return (ll)((ll)r-(ll)l)*(ll)-lazy_val;
		return cur_val + (ll)((ll)r-(ll)l)*lazy_val;
	}
