...
constexpr int SZ = 17;
constexpr int MX = 1 << SZ;
vector<Mint> dp[MX];
Mint ans[MX];

void solve()
{
	for (int i : rep(MX)) dp[i].resize(SZ + 1);
	int n;
	cin >> n;
	string s;
	cin >> s;
	...
	for (int i : rep(n)) {
		...
		for (int l : rep(min(i, n - 1 - i) + 1)) {
			...
			if (s[L] == s[R]) {
				...
				if (s[L] == '?') reqMask |= 1 << (s[R] - 'a');
				else if (s[R] == '?') reqMask |= 1 << (s[L] - 'a');
				else break;
			}
			dbg(i, reqMask, cnt);
			for (int i : rep(18)) {
				dp[reqMask][i] += pow(Mint(i), cnt + unused);
			}
		}
	}
	// cenetered between i, i + 1
	for (int i : rep(n - 1)) {
		int cnt = 0;
		int reqMask = 0;
		int unused = tot;
		for (int l : rep(min(i, n - 2 - i) + 1)) {
			int L = i - l;
			int R = i + l + 1;
			if (s[L] == '?') unused--;
			if (s[R] == '?' && L != R) unused--;
