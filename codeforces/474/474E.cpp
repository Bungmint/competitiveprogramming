...
struct Max {
	int x;
	int id;
	Max(int x_ = 0, int i = -1) : x(x_), id(i) {}
	Max operator+(const Max& other) const {
		if (x > other.x) return *this;
		else return other;
	}
};
...
void solve()
{
	...
	cin >> n >> d;
	vl h(n), ind;
	for (auto &e : h) cin >> e, ind.pb(e), ind.pb(e + d), ind.pb(e - d);
	sort(all(ind)), ind.resize(unique(all(ind)) - ind.begin());
	SegTree<Max> st(sz(ind));
	auto get = [&](ll x) {
	...
	F0R(i, n) {
		...
		int cur = get(h[i]);
		dbg(lo, hi, cur);
		Max x = st.query(hi, sz(ind)), y = st.query(0, lo + 1);
		dbg(x.x, x.id, y.x, y.id);
		if (x.x > y.x) dp[i] = x.x + 1, par[i] = x.id;
		else dp[i] = y.x + 1, par[i] = y.id;
		Max nod = Max(dp[i], i);
		st.upd(cur, nod);
		if (ckmax(ans, dp[i])) id = i;
	}
	vi res;
	dbg(par);
	while (id != -1) {
