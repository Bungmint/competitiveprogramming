...
void solve()
{
	...
	for (int i : rep(1, n + 1)) {
		for (int j : rep(1, m + 1)) {
			pref[i][j] = pref[i][j - 1] + abs(i - a[j]);
			dbg(pref[i][j], i, j);
		}
	}
	for (int i : rep(MAXN))	for (int j : rep(MAXN)) for (int k : rep(MAXN)) dp[i][j][k] = c[i][j][k] = 2 * INF;
	dp[0][0][m] = 0;
	for (int i : per(m + 1)) c[0][0][i] = min(c[0][0][i + 1], dp[0][0][i]);
	int ans = 2 * INF;
	for (int i : rep(1, m + 1)) {
		for (int cnt : rep(1, n + 1)) {
			for (int j : per(1, i + 1)) {
				int w = c[i - j][cnt - 1][j];
				if (w == 2 * INF) continue;
				ckmin(dp[i][cnt][j], w + pref[cnt][i] - pref[cnt][i - j]);
			}
			for (int j : per(1, i + 1)) {
				c[i][cnt][j] = min(c[i][cnt][j + 1], dp[i][cnt][j]);
				if (i == m) ckmin(ans, dp[i][cnt][j]);
			}
		}
	}
	cout << ans << '\n';
