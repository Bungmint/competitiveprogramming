...
T ray_dist(const Point& p, const Line& l) {
	if (sgn(dot(l.fi, p, l.se)) <= 0) return abs(p-l.fi);
    return line_dist(p, l);
}

bool on_ray(const Point& p, const Line& l) {
	return sgn(cross(p, l.fi, l.se)) == 0 &&
	(sgn(dot(p, l.fi, l.se)) <= 0 || sgn(dot(l.se, p, l.fi)) <= 0);
}
...
void solve() {
	...
	vP v = line_intersect(r, s);
	dbg(v[0]);
	if (sz(v) == 1 && on_ray(v[0], r) && on_ray(v[0], s)) {
		cout << 0 << '\n';
		return;
	}
	ld ans = LINF;
	ckmin(ans, ray_dist(r.fi, s));
	ckmin(ans, ray_dist(s.fi, r));
	cout << ans << '\n';
}
