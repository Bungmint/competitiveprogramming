...
void solve()
{
	...
	F0R(i, n){
		if (!i||a[i-1]<=a[i]) L[i] = 0;
		else L[i] = L[i-1]+1;
	}
	R0F(i, n){
		if (i==n-1||a[i+1]<=a[i]) R[i] = 0;
		else R[i] = R[i+1] + 1;
	}
	dbg(L, R);
	function<bool(int,int, int)> rec = [&](int l, int r, int last){
		...
		if (a[l]<=a[r]){
			...
			if (a[r]>last&&(k&1)) return 1;
			else if (a[l]>last) return (rec(l+1, r, a[l])^1);
			else return 0;
		}else{
			int k = min(r-l+1, R[l]+1);
			if (a[l]>last&&(k&1)) return 1;
			else if (a[r]>last) return (rec(l, r-1, a[r])^1);
			else return 0;
		}
	};
