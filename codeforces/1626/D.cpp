...
void solve()
{
	...
	if (sz(comp) >= 2) {
		int sum = 0;
		for (int i : rep(sz(comp))) {
			sum += comp[i];
			ckmin(ans, calc(sum) + calc(n - sum) + 1);
		}
	}
	// triple
	if (sz(comp) >= 3) {
		int sum = 0;
		vector<multiset<pii>> st(21);
		for (int i : rep(sz(comp))) {
			sum += comp[i];
			if (sum && n - sum) {
				int z = 31 - __builtin_clz(n - sum);
				if ((1 << z) != n - sum) z++;
				st[z].insert({sum, n - sum});
			}
		}
		sum = 0;
		for (int i : rep(sz(comp))) {
			...
			for (int j : rep(21)) {
				auto &s = st[j];
				while (sz(s) && s.begin()->fi <= sum) s.erase(s.begin());
				if (sz(s)) {
					auto [x, y] = *s.begin();
					ckmin(ans, cur + calc(x - sum) + calc(y));
				}
			}
		}
	}
