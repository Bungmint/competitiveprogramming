...
inline namespace Hash{
	const int N = 5e3;
	const Mint B = 9973;
	Mint hsh[N + 2], hshs[N + 2];
	Mint pow[N + 1];
	void precalc() {
		pow[0] = 1;
		FOR(i, 1, N + 1) pow[i] = pow[i - 1] * B;
	}
	void hashString(string &s) {
		int n = sz(s);
		precalc();
		F0R(i, n) hsh[i + 1] = hsh[i] * B + s[i];
		R0F(i, n) hshs[i + 1] = hshs[i + 2] * B + s[i];
	}
	Mint rangeHash(int i, int j) {
		dbg(i, j, hsh[j + 1] - hsh[i] * pow[j - i + 1]);	
		return hsh[j + 1] - hsh[i] * pow[j - i + 1];
	}
	bool isPal(int i, int j) {
		return rangeHash(i, j) == (hshs[i + 1] - hshs[j + 2] * pow[j - i + 1]);
	}
}
...
void solve()
{
	...
	FOR(len, 2, 1 + n) {
		F0R(l, n - len + 1) {
			...
			if (!isPal(l, r)) {
				dp[l][r] = 0;
			}else if (rangeHash(l, l + half - 1) == rangeHash(r - half + 1, r)) {
				dp[l][r] = 1 + dp[l][l + half - 1];
			}
			ans[dp[l][r] + 1]--;
			ans[0]++;
		}
	}
	FOR(i, 1, n + 2) ans[i] += ans[i - 1];
