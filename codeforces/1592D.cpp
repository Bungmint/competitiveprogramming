...
void solve()
{
	...
	while(l<=r){
		...
		a.clear();
		F0R(i, mid+1){
			st.insert(edges[i].fi), st.insert(edges[i].se);
		}
		for (auto x:st) a.pb(x);
		if (ask(a)==total) ans = mid, r=mid-1;
		else l = mid +1;
	}
	cout << "! "<< edges[ans].fi << " "<< edges[ans].se<<endl;
