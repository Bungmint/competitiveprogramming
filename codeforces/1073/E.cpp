...
pair<ll, Mint> dfs(int pos, int mask, bool fl, bool nonzr) {
	if (pos == sz(num)) {
		if (__builtin_popcount(mask) <= k && nonzr) {	
			return {1, 0};
		}else return {0, 0};
	}
	ll& state = dp[pos][mask][fl][nonzr];
	Mint& su = sum[pos][mask][fl][nonzr];
	if (state != -1) return {state, su};
	state = 0;
	int LMT = (fl ? 9 : num[pos]);
	for (int i = 0; i <= LMT; i++) {
		auto [cnt, t] = dfs(pos + 1, (nonzr || i ? (mask | (1 << i)) : mask), fl || (i < LMT), nonzr || !!i);
		state += cnt;
		su += Mint(cnt) * powt[sz(num) - pos - 1] * i;
		su += t;
	}
	return {state, su};
}
...
void solve()
{
	cin >> l >> r >> k;
	powt[0] = 1;
	for (int i : rep(1, 22)) powt[i] = powt[i - 1] * 10;
	Mint rans{}, lans{};
	init(r);
	rans = dfs(0, 0, 0, 0).se;
	init(l - 1);
