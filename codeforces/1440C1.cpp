...
void solve()
{
	...
	FOR(i, n-2, n)FOR(j, m-2, m){
		if (grid[i][j]) on.insert({i+1, j+1});
		else off.insert({i+1,j+1});
	}
	while(sz(on)){
		vpi tmp;
		if (sz(on)==3){
			for (auto &p:on) tmp.pb(p), off.insert(p);
			on.clear();
			ans.pb(tmp);
		}else if (sz(on)<=2&&sz(on)>=1){
			F0R(i, 2){
				tmp.pb(*off.begin());
				off.erase(off.begin());
			}
			tmp.pb(*on.begin());
			off.insert(tmp[2]);
			on.insert(tmp[0]), on.insert(tmp[1]);
			on.erase(tmp[2]);
			ans.pb(tmp);
		}else{
			F0R(i, 3){
	...
	cout << sz(ans)<<"\n";
	for (auto x: ans) {for (auto [a, b]:x) cout << a << " "<<b << " ";cout << "\n";}
}
