...
ll bin_pow(ll a, ll b){
	if (b==1) return a;
	ll x = bin_pow(a, b>>1LL);
	return (b&1) ? x*x*a:x*x;
}
bool hi(ld a, ld b){return a-b>1e-9;}
...
void solve()
{
	...
	F0R(i, n){
		if (prev!=-1&&rem){
			ll k = prev-a[i];
			ld val = (ld)k * (ld)log10((ld)p) + (ld)log10((ld)rem);
			if (hi(val, 18.5)) rem = LINF;
			else rem = bin_pow(p, k)*rem;
			dbg(val);
		}
		ll cnt = 1;
		dbg(res);
		...
		if (rem==0){
			if (cnt%2==1) rem = 1LL, res += pow((Mint)p, a[i]);
		}else{
			dbg(cnt, rem);
			if (cnt>=rem){
				res =0;
				ll x = cnt-rem;
				rem = 0;
				if (x%2==1) rem = 1LL, res += pow((Mint)p, a[i]);
				dbg(res);
			}else{
				res -= pow((Mint)p, a[i])*cnt;
				rem -= cnt;
			}
		}
		prev = a[i];
