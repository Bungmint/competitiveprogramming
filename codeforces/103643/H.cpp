...
void solve()
{
	int n, m;
	cin >> n >> m;
	vector<vector<char>> island(n + 1, vector<char>(m + 1));
	for (auto &v : island) for (auto &e : v) cin >> e;
	vpi a, b, c;
	vector<map<pii, int>> aa;
	for (int i : rep(n + 1)) for (int j : rep(m + 1)) {
		char x = island[i][j];
		if (x == 'A') a.pb({i, j});
		if (x == 'B') b.pb({i, j});
		if (x == 'C') c.pb({i, j});
	...
	for (int i : rep(sz(a))) {
		...
		for (auto [z, w] : c) {
			...
			k /= g, l /= g;
			if (aa[i].count({k, l})) {
				ckmin(aa[i][{k, l}], abs(z - x) + abs(w - y));
			}else aa[i][{k, l}] = abs(z - x) + abs(w - y);
		}
		dbg(aa[i]);
		for (auto [z, w] : b) {
			int k = z - x, l = w - y;
			...
			dbg(k, l);
			if (aa[i].count({k, l})) {
				if (abs(z - x) + abs(w - y) < aa[i][{k, l}]) ans++;
			}else ans++;
		}
		dbg(ans);
	}
