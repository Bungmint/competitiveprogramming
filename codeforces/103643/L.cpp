...
void solve()
{
	...
	cin >> n >> r >> m >> k;
	vi a(n), ind;
	vector<ar<int, 4>> query(r);
	for (int &i : a) cin >> i, ind.pb(i);
	for (auto &[x, y, z, w] : query) cin >> x >> y >> z >> w, x--, y--, ind.pb(z);
	sort(all(ind));
	ind.resize(unique(all(ind)) - ind.begin());
	BIT<int> bit(sz(ind));
	for (int &i : a) i = lb(all(ind), i) - ind.begin();
	for (auto &[x, y, z, w] : query) z = lb(all(ind), z) - ind.begin();
	auto good = [&](int x) -> bool {
		return x <= k || x >= n - k;
	};
	for (int i : rep(1, n)) {
		if (good(i)) bit.upd(a[i] + 1, 1);
	}
	for (int i : rep(r)) {
		auto &[x, y, z, w] = query[i];
		bool xx = good(x), yy = good(y);
		if (xx == yy) {
			swap(a[x], a[y]);
		}else{
			if (xx) swap(xx, yy), swap(x, y);
			bit.upd(a[x] + 1, 1);
			bit.upd(a[y] + 1, -1);
			swap(a[x], a[y]);
		}
		int xxx = bit.rangeQuery(z + 2, sz(ind));
		if (xxx >= k) {
			m += w;
		}else m -= w;
		if (m <= 0) {
