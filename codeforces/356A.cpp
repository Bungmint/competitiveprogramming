...
void solve()
{
    ...
    for (int i = m - 1; i >= 0; i--)
    {
        st.modify(op[i][0], op[i][1] + 1, op[i][2]);
        int v = st2.get(op[i][2]);
        if (v == 0 || v == op[i][2])
            st.modify(op[i][2], op[i][2] + 1, 0);
        else
            st.modify(op[i][2], op[i][2] + 1, v);
        st2.modify(op[i][0], op[i][1] + 1, op[i][2]);
        if (v == 0 || v == op[i][2])
            st2.modify(op[i][2], op[i][2] + 1, 0);
        else
            st2.modify(op[i][2], op[i][2] + 1, v);
    }
    for (int i = 1; i <= n; i++)
    {
        cout << st.get(i) << " ";
