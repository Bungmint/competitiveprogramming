...
void solve()
{
	vi lp(71, INF), is_pr(71, 1), ind(71);
	is_pr[0] = is_pr[1] = 0;
	int co{};
	for (int i : rep(2, 71)) {
		if (is_pr[i]) {
			lp[i] = i;
			ind[i] = co++;
			for (int j = 2 * i; j <= 70; j += i) is_pr[j] = 0, ckmin(lp[j], i);
		}
	}
	dbg(count(all(is_pr), 1));
	int n;
	cin >> n;
	vi a(n), todo;
	vmi dp(1 << 19), nxt(1 << 19);
	dp[0] = 1;
	vi cnt(71);
	...
	for (int i : rep(1, 71)) {
		...
		while (ii > 1) {
			int p = lp[ii];
			int cn{};
			while (ii % p == 0) ii /= p, cn++;
			if (cn & 1) msk ^= (1 << ind[p]);
		}
		dbg(msk);
		Mint ha = pow((Mint)2, cnt[i] - 1);
		nxt.assign(1 << 19, 0);
		for (int mask : rep(1 << 19)) {
			nxt[mask] += (dp[mask] + dp[mask ^ msk]) * ha;
		}
		dbg(nxt[0]);
		swap(nxt, dp);
	}
