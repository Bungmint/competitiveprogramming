...
struct Min {
	ll v;
	Min(ll v_ = numeric_limits<ll>::max()) : v(v_) {}
...
void solve()
{
	...
	F0R(r, n) {
		for (auto &l : todo[r]) {
			ll z = (x[r] - x[l]) * (a[r] + a[l]);
			t.upd(l, z);
		}
		for (auto &[l, id] : query[r]) {
			ans[id] = t.query(l, r + 1).v;
		}
	}
