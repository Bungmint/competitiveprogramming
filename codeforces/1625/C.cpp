...
void solve()
{
	...
	for (auto &e : d) cin >> e;
	for (auto &e : a) cin >> e;
	vector<vi> dp(n, vi(k + 1, 2 * INF)), nxt(n, vi(k + 1, 2 * INF));
	dp[0][0] = 0;
	for (int i : rep(1, n)) {
		for (auto &v : nxt) {
			for (auto &e : v) e = 2 * INF;
		}
		// remove i
		for (int last : rep(i)) {
			for (int remcnt: rep(k)) {
				if (dp[last][remcnt] == 2 * INF) continue;
				ckmin(nxt[last][remcnt + 1], dp[last][remcnt] + a[last] * (d[i] - d[i - 1]));
			}
		}
		// keep i
		for (int last : rep(i)) {
			for (int remcnt: rep(k + 1)) {
				if (dp[last][remcnt] == 2 * INF) continue;
				ckmin(nxt[i][remcnt], dp[last][remcnt] + a[last] * (d[i] - d[i - 1]));
			}
		}
		swap(dp, nxt);
	}
	...
	for (int last : rep(n)) {
		for (int remcnt: rep(k + 1)) {
			if (dp[last][remcnt] == 2 * INF) continue;
			dbg(last, remcnt, dp[last][remcnt]);
			ckmin(ans, dp[last][remcnt] + a[last] * (l - d[n - 1]));
		}
	}
	cout << ans << '\n';
}
