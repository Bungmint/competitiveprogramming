...
void solve() {
	...
	vmi num_paths(n);
	vl literal_paths(n);
	vector<bool> is_pos(n);
	num_paths[en] = 1; is_pos[en] = 1; literal_paths[en] = 1;
	for (int dist = 1; dist <= n - 1; dist++) {
		vmi nxt(n);
		vl nxt_paths(n);
		vector<bool> nxt_pos(n);
		for (int u = 0; u < n; u++) {
			for (auto &v : adj[u]) {
				nxt[u] += num_paths[v];
				nxt_paths[u] += literal_paths[v];
				nxt_pos[u] = nxt_pos[u] || is_pos[v];
			}
			if (nxt_paths[u] >= 2000) nxt_paths[u] = 2000;
		}
		swap(nxt, num_paths);
		swap(is_pos, nxt_pos);
		swap(literal_paths, nxt_paths);
		ll sums{};
		for (int u = 0; u < n; u++) {
			if (is_pos[u]) {
				sums += 1LL * a[u] * literal_paths[u];
				ans += num_paths[u] * a[u];
			}
		}
		if (sums >= 2000) sums = 2000;
		if (sums == 0) {
			continue;
		}
		if (la < dist) {
