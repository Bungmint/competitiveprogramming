...
void solve()
{
	int n, k;
	cin >> n >> k;
	vector<vector<Mint>> dp(n + 1, vector<Mint>(n + 1)); // rows; columns covered
	dp[0][0] = 1;
	FOR(row, 1, n + 1) {
		FOR(cov, 0, n + 1) {
			FOR(previous, 0, cov + 1) {
				int add = cov - previous;
				if (add) dp[row][cov] += dp[row - 1][previous] * pow((Mint)k, previous) * nCk(n - previous, add) * pow((Mint)(k - 1), n - cov);
				else dp[row][cov] += dp[row - 1][previous] * (pow((Mint)k, previous) - pow((Mint)(k - 1), previous)) * pow((Mint)(k - 1), n - cov);
			}
		}
	}
	cout << dp[n][n] << "\n";
