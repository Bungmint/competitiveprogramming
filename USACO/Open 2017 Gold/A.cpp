...
void precalc() {
	power[0] = 1;
	FOR(i, 1, N + 1) power[i] = power[i - 1] * B;
}
...
void solve()
{
	...
	F0R(i, 2 * n) {
		string s;
		cin >> s;
		F0R(j, m) hsh[i][j + 1] = hsh[i][j] * B + s[j];
	}
	int ans = m;
	int l = 1, r = m;
	while (l <= r) {
		...
		F0R(i, m - mid + 1) {
			set<int> st;
			F0R(k, n) {
				st.insert((int)(hsh[k][i + mid] - hsh[k][i] * power[mid]));
			}
			bool ok = 1;
			F0R(k, n) {
				if (st.count((int)(hsh[k + n][i + mid] - hsh[k + n][i] * power[mid]))) ok = 0;
			}
			fl |= ok;
		}
		if (fl) ans = mid, r = mid - 1;
		else l = mid + 1;
	}
	cout << ans << "\n";
}
