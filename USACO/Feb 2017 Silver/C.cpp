...
void solve()
{
	...
	for (auto&[x, y]:cows) cin >> x >> y, x--, y--;
	function<void(int, int, int)> dfs = [&](int r, int c, int ccc){
		if (r<0||c<0||r>=n||c>=n) return;
		if (comp[r][c]) return;
		comp[r][c] = ccc;
		F0R(i, 4){
			int nr = r + dx[i], nc = c + dy[i];
			if (st.count({r, c, nr, nc})) continue;
			dfs(nr, nc, ccc);
		}
	};
	F0R(i, n) F0R(j, n){
