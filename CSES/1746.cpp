...
void solve()
{
	int n, m;
	cin >> n >> m;
	vector<vector<Mint>> dp(2, vector<Mint>(m));
	...
	F0R(i, n){
		...
		if (!i){
			if (p){
				...
				F0R(j, m) dp[current][j] = 1;
			}
		}else{
			FOR(j, 0, m) dp[current][j] = 0;
			if (p){
				p--;
				FOR(j, p - 1, p + 2) if (j >= 0 && j < m) dp[current][p] += dp[previous][j];
			}else{
				FOR(j, 0, m){
					if (j) dp[current][j] += dp[previous][j - 1];
					if (j + 1 < m) dp[current][j] += dp[previous][j + 1];
	...
	dbg(dp);
	cout << accumulate(all(dp[previous]), (Mint)0) << "\n";
}
