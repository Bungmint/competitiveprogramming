...
void solve()
{
	...
	for (auto &e : a) e = getInd(e);
	vector<vi> colorIndex(sz(ind));
	REP(n - 1) {
		int u, v;
		cin >> u >> v;
		u--, v--;
	...
	dfs(0, -1);
	vi color(timer);
	F0R(i, n) color[tin[i]] = a[i], colorIndex[a[i]].pb(tin[i]);
	dbg(color);
	F0R(i, sz(ind)) sort(all(colorIndex[i]), greater<int>());
	vi order(n);
	iota(all(order), 0), sort(all(order), [&](int u, int v) {
		return make_pair(tin[u], tout[u]) < make_pair(tin[v], tout[v]);
	});
	SegTree<int> st(timer);
	F0R(i, sz(ind)) st.upd(colorIndex[i].back(), 1);
	int cur = 0;
	F0R(i, timer) {
		while (cur < n && tin[order[cur]] == i) {
			int v = order[cur];
			ans[v] = st.query(tin[v], tout[v] + 1);
			cur++;
		}
		st.upd(colorIndex[color[i]].back(), 0);
		dbg(i, colorIndex[color[i]]);
		colorIndex[color[i]].pop_back();
		if (sz(colorIndex[color[i]])) st.upd(colorIndex[color[i]].back(), 1);
	}
