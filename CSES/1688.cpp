...
struct Node{
	int vertex;
	Node() : vertex(-1) {}
	Node(int v_) : vertex(v_) {}
	Node operator+(const Node& other) const{
		if (vertex == -1) return other.vertex;
		if (other.vertex == -1) return vertex;
		return (tin[vertex] < tin[other.vertex] ? vertex : other.vertex);
	}
};
...
void solve()
{
	...
	SegTree<Node> st(2 * n);
	dfs(0, -1);
	F0R(i, timer) st.upd(i, eulerTour[i]);
	REP(q) {
		int u, v;
		cin >> u >> v;
		u--, v--;
		if (tin[u] > tin[v]) swap(u, v);
		cout << st.query(tin[u], tin[v] + 1).vertex + 1 << "\n";
	}
}
