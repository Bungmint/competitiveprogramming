...
void solve()
{
	...
	int x = ek.maxFlow(1,n);
	F0R(i, n+1) F0R(j, n+1) {cap[i][j] -= ek.cap[i][j]; if (cap[i][j]>0) g[i].pb(j);}
	cout <<x<<"\n";
	queue<int> q;
	REP(x){
		...
		st.push(1);
		cout << sz(st)<<"\n";
		while(sz(st))cout << st.top()<<" ", st.pop();
		cout << "\n";
	}
