...
void solve()
{
	...
	lcp_construction(S);
	SparseTable<int, 17> spar(lcp, n-1);
	vl pref(n);
	F0R(i, n)pref[i] = (i? pref[i-1]+n-sa[i]:n-sa[i]);
	ll k;
	cin >> k;
	string ans = "";
	ll sum = 0;
	...
	F0R(i, n){
		...
		if (i){
			// No add
			int st = last+1;
			int l = st+1, r = n-1, en=st;
			while(l<=r){
				int m = l + (r-l)/2;
				if (spar.query(st, m-1)>=len) en = m, l = m+1;
				else r = m-1;
			}
			if (sum+en-st+1>=k){
				break;
			...
			while(true){
				...
				while(l<=r){
					int m = l + (r-l)/2;
					if (spar.query(st, m-1)>=len+1) en = m, l = m+1;
					else r = m-1;
				}
				dbg(st,en);
				ll val = pref[en];
				...
				val-=(en-st+1)*len;
				dbg(val);
				if (sum+val>=k) {ans += (char)(S[sa[st]+len]);break;}
				else last=en, sum+=val;
			}
		}else{
			while(true){
				int st = last+1;
				...
				while(l<=r){
					int m = l + (r-l)/2;
					if (spar.query(st, m-1)) en = m, l = m+1;
					else r = m-1;
				}
				ll val = pref[en];
				if (st) val-=pref[st-1];
				if (sum+val>=k) {ans += (char)(S[sa[st]]);break;}
				else last = en, sum+=val;
			}
		}
	}
