...
void solve()
{
	...
	vi ans(n);
	multiset<ll> st;
	for (auto &e : b) cin >> e, st.insert(e);
	sort(all(b));
	ll x = b[0], y = b[1];
	auto good = [&](ll x) -> bool {
		return x >= 1 && x <= 1e9;
	...
	for (int i : rep(2, n)) {
		...
		if ((x + y + z) & 1) continue;
		ll w = (x + y + z) / 2;
		ll a1 = w - z, a2 = w - y, a3 = w - x;
		if (!good(a1) || !good(a2) || !good(a3)) {
			continue;
		}
		ans[0] = a1, ans[1] = a2, ans[2] = a3;
		multiset<ll> tmp = st;
		dbg(tmp);
		tmp.erase(tmp.find(x)), tmp.erase(tmp.find(y)), tmp.erase(tmp.find(z));
		bool ok = 1;
		for (int j : rep(3, n)) {
			ll c = *tmp.begin();
			tmp.erase(tmp.begin());
			...
			for (int l : rep(1, j)) {
				ll t = ans[j] + ans[l];
				if (auto it = tmp.find(t); it == tmp.end()) {
					ok = 0;
					break;
				}else{
					tmp.erase(it);
