...
unordered_map<int,int,chash> mp[5050];
Mint dp[5050];
void solve()
{
	string s;
	cin >> s;
	FOR(i,1,sz(s)+1)hsh[i] = hsh[i-1]+power[i-1]*(s[i-1]-'a'+1);
	int k;
	cin >> k;
	REP(k){
		string t;
		...
		Mint hash=0;
		F0R(i,sz(t))hash += power[i]*(t[i]-'a'+1);
		dbg(hash);
		mp[sz(t)][(int)hash]++;
	}
	dp[0] = 1;
	FOR(i,1,sz(s)+1){
		ROF(j,1,i+1){
			int x = (int)((hsh[i]-hsh[i-j])*inverse[i-j]);
			if (mp[j].count(x))dp[i]+=dp[i-j];
		}
	}
	cout << dp[sz(s)]<<endl;
}
