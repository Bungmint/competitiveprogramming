...
Mint hashing(int i, int len){
	int j = (i+len-1)%n;
	if(j<i) return (hsh[n]-hsh[i])*inverse[i] + hsh[j+1]*power[n-i];
	else return (hsh[j+1]-hsh[i])*inverse[i];
}
...
void solve()
{
	...
	F0R(i, n){
		...
		while(l<=r){
			int m = l+(r-l)/2;
			if (hashing(i,m)==hashing(id,m)){
				len = m;
				l = m+1;
			}else r = m-1;
		}
		if (len == n) continue;
		if (s[(i+len)%n]<s[(id+len)%n]) id = i;
	}
	F0R(i,n) cout << s[(id+i)%n];
}
