...
void solve()
{
	...
	cin >> h>>w>>n;
	vpi block(n);
	F0R(i, n)cin >> block[i].fi >> block[i].se, block[i].fi--, block[i].se--;
	block.pb({h-1, w-1});
	vector<Mint> dp(n+1);
	sort(all(block), [&](const pii& lhs, const pii& rhs){
	...
	F0R(i, n+1){
		...
		dp[i] = nCk(r+c, r);
		F0R(j, i){
			auto [befR, befC] = block[j];
			if (befR<=r&&befC<=c){
				dp[i]-=dp[j]*nCk(r+c-befR-befC, r-befR);
			}
		}
	}
	cout << dp[n]<<endl;
