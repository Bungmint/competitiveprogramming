...
Mint dping(int L, int R){
	if (vis[L][R]) return dp[L][R];
	Mint&res = dp[L][R];
	if (R<L) {vis[L][R] = 1;return res = 1;}
	FOR(i, L+1, R+1){
		if ((i-L)%2==0||!good[i][L]) continue;
		Mint x = dping(L+1, i-1), y = dping(i+1, R);
		int xLen = (i-L-1)/2, yLen = (R-i)/2;
		res += (Mint)x*(Mint)y*nCk(xLen+1+yLen, yLen);
	}
	vis[L][R] = 1;
	return res;
}
