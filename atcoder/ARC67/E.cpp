...
void solve() {
	...
	for (int i = 1; i <= SQ; i++) {
		if (i > b || i < a) continue;
		nxt = dp; // when we take no group of size i
		for (int gs = c; gs <= d; gs++) {
			for (int j = gs*i; j <= n; j++) {
				// dbg(j, gs, i);
				nxt[j] += dp[j - gs*i] * C.binom(n - j + gs*i, gs*i)
						* C.fact[gs*i] * pow(C.invFact[i], gs)
						/ C.fact[gs];
			}
		}
		// dbg(nxt);
