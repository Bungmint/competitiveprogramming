...
bool is_inc(const pll& iv) {
    return iv.first <= iv.second;
}
bool is_dec(const pll& iv) {
    return iv.first >= iv.second;
}
...
struct Node {
    ...
    Node operator+(const ll& x) {
        left.fi += x, left.se += x;
        right.fi += x, right.se += x;
        dbg(left, right, sum, x);
        return *this;
    }
    Node operator+(const Node& oth) const {
        if (cnt == 0) return oth;
        if (oth.cnt == 0) return *this;
        Node res{};
        res.sum = sum + oth.sum;
        res.left = left, res.right = oth.right;
        res.cnt = cnt + oth.cnt;
        if (is_inc(right) && is_inc(oth.left)) {
            res.cnt--;    
            res.sum += oth.left.fi - right.se;
            if (cnt == 1) {
                res.left = {right.fi, oth.left.se};
            }
            if (oth.cnt == 1) {
                res.right = {right.fi, oth.left.se};
            }
        }else if (is_dec(right) && is_dec(oth.left)) {
            res.cnt--;
            res.sum += right.se - oth.left.fi;
            if (cnt == 1) {
                res.left = {right.fi, oth.left.se};
            }
            if (oth.cnt == 1) {
