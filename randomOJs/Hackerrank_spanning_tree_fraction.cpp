...
void solve()
{
	...
	cin >> n >> m;
	vector<ar<int, 4>> g(m);
	for (auto &[u, v, a, b] : g) cin >> u >> v >> a >> b;
	int l = 1e5, r = 1e9, ans = 0;
	int p, q;
	while (l <= r) {
		int mid = (l + r) / 2;
		vE edges;
		for (auto &[u, v, a, b] : g) edges.pb({u, v, a * (int)1e7 - 1LL * mid * b, a, b});
		sort(all(edges));
		DSU dsu(n);
		ll totW{};
		int curP = 0, curQ = 0;
		for (auto &[u, v, w, a, b] : edges) {
			if (dsu.unite(u, v)) {
				totW += w;
				curP += a, curQ += b;
			}
		}
		if (totW >= 0) ans = mid, l = mid + 1, p = curP, q = curQ;
		else r = mid - 1;
	}
	int gg = gcd(p, q);
	p /= gg, q /= gg;
