...
struct Dinic {
    ...
    vi ptr, lvl;
    Dinic(int _N) { N = _N; adj.resize(N); is_forward.resize(N); }
    pii add_edge(int a, int b, F cap, F rcap = 0) {
        assert(min(cap, rcap) >= 0);
        adj[a].pb({b, sz(adj[b]), cap});
		is_forward[a].pb(true);
		is_forward[b].pb(false);
	...
	void dfs_path(int u) {
		...
		for (int &i = ptr[u]; i < sz(adj[u]); i++) {
			if (get_edge_flow(make_pair(u, i)) && 
				is_forward[u][i]) {
				i++;
				dfs_path(adj[u][i - 1].to);
				break;
			}
		}
	}
	vector<vector<bool>> is_forward;
	vector<vi> get_paths(int s, int t) {
		ptr = vi(N);
		vector<vi> ret;
		for (int i = 0; i < sz(adj[s]); i++) {
			if (get_edge_flow(make_pair(s, i)) && is_forward[s][i]) {
				res = {s};
				dfs_path(adj[s][i].to);
				if (res.back() != t) break;
				ret.pb(res);
...
void solve() {
	...
	vector<vi> ans = dinic.get_paths(s, t);
