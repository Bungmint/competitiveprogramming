...
void solve()
{
	...
	int sti, stj, eni, enj;
	F0R(i, n) F0R(j, m) {
		if (grid[i][j] == 'A') sti = i, stj = j;
		if (grid[i][j] == 'B') eni = i, enj	= j;
	}
	vector<vi> d(n, vi(m, INF));
	vector<vpi> par(n, vpi(m));
	...
	while (sz(q)) {
		...
		F0R(i, 4) {
			...
			if (ckmin(d[nx][ny], d[x][y] + 1)) {
				q.push({nx, ny});
				par[nx][ny] = {x, y};
				backtrack[nx][ny] = dir[i];
			}
		}
	}
	if (d[eni][enj] == INF) cout << "NO";
	else{
		cout << "YES" << "\n";
		cout << d[eni][enj] << "\n";
		string ans = "";
