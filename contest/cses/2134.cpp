...
void solve()
{
	...
	cin >> n >> q;
	vector<vi> g(n);
	vi par(n, -1), depth(n), heavy(n, -1), head(n), pos(n), subt(n);
	vector<Max> t(n);
	vi vv(n);
	SegTree<Max> st(n);
	int co{};
	...
	auto build = [&]() -> void {
		for (auto i : rep(n)) t[pos[i]] = vv[i];
		st.build(t);
	};
	auto upd = [&](int u, int x) -> void {
		st.upd(pos[u], x);
	};
	auto query = [&](int a, int b) -> int {
		...
		for (; head[a] != head[b]; b = par[head[b]]) {
			...
			ckmax(res, st.query(pos[head[b]], pos[b] + 1).x);
		}
		if (depth[a] > depth[b]) swap(a, b);
		ckmax(res, st.query(pos[a], pos[b] + 1).x);
		return res;
	};
