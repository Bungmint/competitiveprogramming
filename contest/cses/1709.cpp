...
void solve() {
    ...
    vpi edges;
    for (int i = 1; i <= N; i++) {
        edges.pb(dinic.add_edge(st, i, 1));
        edges.pb(dinic.add_edge(i + N, en, 1));
    }
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            char c;
            cin >> c;
            if (c == 'o') {
                dinic.add_edge(1 + i, 1 + j + N, INF);
            }
        }
    }
    int ans = dinic.get_max_flow(st, en);
    bool res = dinic.bfs(st, en);
    dbg(res);
    vi rs, cs;
    assert(dinic.lvl[en] == 0);
    for (int i = 1; i <= N; i++) {
        if (dinic.lvl[i] == 0) rs.pb(i);
        if (dinic.lvl[i + N] != 0) cs.pb(i);
    }
    cout << ans << '\n';
    for (auto &e : rs) cout << 1 << ' ' << e << '\n';
    for (auto &e : cs) cout << 2 << ' ' << e << '\n';
}
