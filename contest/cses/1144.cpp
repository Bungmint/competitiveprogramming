...
void solve()
{
	int n, q;
	cin >> n >> q;
	vector<pair<char, pii>> quer(q);
	vi a(n), order;
	for (auto &e : a) cin >> e, order.pb(e);
	F0R(i, q) {
		cin >> quer[i].fi >> quer[i].se.fi >> quer[i].se.se;
		if (quer[i].fi == '!') order.pb(quer[i].se.se);
		else order.pb(quer[i].se.fi), order.pb(quer[i].se.se);
	}
	sort(all(order)), order.resize(unique(all(order)) - order.begin());
	SegTree<int> st(sz(order));
	for (auto &e : a) {
	...
	for (auto &[op, p] : quer) {
		auto &[x, y] = p;
		if (op == '?') {
			x = lb(all(order), x) - order.begin();
			y = lb(all(order), y) - order.begin();
			cout << st.query(x, y + 1) << "\n";
		}else{
			x--;
			y = lb(all(order), y) - order.begin();
