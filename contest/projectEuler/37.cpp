...
Sieve<1000'001> sieve;
vi powt(8, 1);

bool isTrunc(int p) {
	int pp = p;
	bool res = 1;
	while (pp) res &= sieve.is_prime[pp], pp /= 10;
	int d = ub(all(powt), p) - powt.begin();
	if (p == 53) dbg(res, p);
	for (int i : per(1, d)) {
		p %= powt[i];
		res &= sieve.is_prime[p];
	}
	return res;
}
...
{
	int ans = 0;
	for (int i : rep(1, 8)) powt[i] = powt[i - 1] * 10;
	for (int i : rep(10, 1000001)) {
		if (isTrunc(i)) {
			ans += i;
			dbg(i);
		}
