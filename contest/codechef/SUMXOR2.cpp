...
void solve()
{
	...
	vi a(n);
	F0R(i, n)cin >> a[i];
	vector<Mint> sum(n+1), pref(n+1);
	F0R(i, 30){ // current bit
		int n0 = 0, n1 = 0;
		F0R(j, n){
			if (a[j]&(1<<i)) n1++;
		}
		n0 = n-n1;
		vector<Mint> polyA(n1+1), polyB(n0+1);
		dbg(n1, n0);
		for (int j=1;j<=n1;j+=2) polyA[j] = nCk(n1, j);
		F0R(j, n0+1) polyB[j] = nCk(n0, j);
		vector<Mint> res = mul(polyA, polyB);
		dbg(polyA, polyB);
		dbg(res);
		FOR(j, 1, n+1) sum[j] += res[j] * (1LL<<i);
		dbg();
	}
	FOR(i, 1, n+1) pref[i] = pref[i-1]+sum[i];
	int q;
	cin >> q;
	while(q--){
		int m;
