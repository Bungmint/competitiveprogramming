...
TrieBit<30*200'010, 30> trie;

int divide_conquer(int L, const vi& a) {
	if (sz(a) == 0) return 0;
	vi one, zero;
	for (auto &x : a) {
		if (x>>L & 1) one.pb(x);
	...
	if (sz(one) & 1) {
		for (auto &x : one) trie.insert(x);
		int ans{2*INF};
		for (auto &x : zero) ckmin(ans, (int)trie.query_xor_min(x));
		return ans;
	}
	if (L == 0) return 0;
	int o = divide_conquer(L-1, one);
	int z = divide_conquer(L-1, zero);
	return max(o, z);
}

void solve() {
	int n; cin >> n;
	vi a(2*n); for (auto &e : a) cin >> e;
	cout << divide_conquer(30, a) << '\n';
}
