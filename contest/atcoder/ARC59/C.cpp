...
void solve()
{
	...
	vvmi dp(n + 1, vmi(c + 1)), powPref(c + 1, vmi(401));
	for (int i : rep(c + 1)) {
		for (int j : rep(1, 401)) powPref[i][j] = powPref[i][j - 1] + pow(Mint(j), i);
	}
	for (int i : rep(1, n + 1)) {
		for (int j : rep(0, c + 1)) {
			if (i == 1) {
				dp[i][j] = powPref[j][b[i - 1]] - powPref[j][a[i - 1] - 1];
			}else{
				for (int k : rep(j + 1)) {
					dp[i][j] += dp[i - 1][k] * (powPref[j - k][b[i - 1]] - powPref[j - k][a[i - 1] - 1]);
				}
			}
		}
	}
