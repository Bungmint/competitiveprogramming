...
const int MX=1e6+3, MK = 26;
int s[MX], sa[MX],ch[MX];
int lcp[MX];
string S;
...
void solve()
{
	...
	R0F(i, n-1){
		...
		r[i] = (sz(st)? st.top():n-1);
		st.push(i);
	}
	R0F(i,n-1){
		f[i] = f[r[i]]+lcp[i]*(r[i]-i);
	}
	FOR(i,1,n){
		g[i] = g[l[i-1]+1] + lcp[i-1]*(i-1-l[i-1]);
	}
	
	
	F0R(i, n)cout << f[ch[i]]+g[ch[i]]+n-sa[ch[i]]<<"\n";
}
