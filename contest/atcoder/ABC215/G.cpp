// Problem: G - Colorful Candies 2
// Contest: AtCoder - AtCoder Beginner Contest 215
// URL: https://atcoder.jp/contests/abc215/tasks/abc215_g
...
void solve()
{
	...
	F0R(i, n) a[i] = lb(all(order), a[i])-begin(order),tmp[a[i]]++;
	F0R(i, n) cnt[tmp[i]]++;
	FOR(i, 1, n+1) if (cnt[i]) p.pb({i, cnt[i]});
	dbg(p);
	FOR(k, 1, n+1){
		Mint ans = 0;
		for (auto [n1, x]:p){
			ans += x*(nCk(n, k)-nCk(n-n1, k));
		}
		cout << ans/nCk(n, k)<<"\n";
	}
}
