...
constexpr int MX = 1e5 + 10, N = 5000;
ll dp[MX];
int par[MX];
int bit[N + 1][N + 1]{};
...
int query(int x, int y) {
	...
	for (x++; x; x -= x & -x) {
		for (int i = y + 1; i; i -= i & -i) {
			if (dp[res] < dp[bit[x][i]]) res = bit[x][i];
		}
	}
	return res;
}
...
void solve()
{
	...
	sort(a, a + n);
	int res{};
	for (auto &[L, W, H, P, id] : a) {
		int pa = query(5000 - L, 5000 - W);
		if (pa == -1) {
			dp[id] = P;
		}else dp[id] = P + dp[pa];
		par[id] = pa;
		upd(5000 - L, 5000 - W, id);
		if (dp[id] > dp[res]) res = id;
	}
	vi ans;
	int head = res;
	while (head) {
