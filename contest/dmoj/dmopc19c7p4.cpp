...
pmi mul(pmi a, int v) {
	return {a.fi * v, a.se * v};
}
pmi add(pmi a, pmi b) {
	return {a.fi + b.fi, a.se + b.se};
}
pmi add(pmi  a, int x) {
	return {a.fi + a.se * x, a.se};
}
pmi inv(pmi b) {
	return {b.se, b.fi};
}
void DnC(int L, int R, vi& query) {
	...
	for (int i : per(L, M)) {
		if (i == M - 1) {
			...
			lef[i].fi = add(inv(lef[i + 1].fi), a[i]);
			lef[i].se = add(inv(lef[i + 1].se), a[i]);
		}
		dbg(lef[i], i, L, R);
	}
	for (int i : rep(M, R)) {
		if (i == M) {
			...
			rig[i].se = rig[i - 1].fi;
			rig[i].fi = add(rig[i - 1].se, mul(rig[i - 1].fi, a[i]));
		}
		dbg(rig[i], i, L, R);
	}
	for (auto &id : query) {
		if (A[id] >= M) rigq.pb(id);
			...
			auto [Q, P] = rig[B[id]].fi;
			auto [f1, f2] = lef[A[id]];
			ans[id] = add(mul(f1, (int)Q), mul(f2, (int)P));
		}
	}
...
void solve()
{
	...
	vi query(q);
	for (int i : rep(q)) cin >> A[i] >> B[i], A[i]--, B[i]--, query[i] = i;
	DnC(0, n, query);
	for (int i : rep(q)) cout << ans[i].fi << ' ' << ans[i].se << '\n';
}
