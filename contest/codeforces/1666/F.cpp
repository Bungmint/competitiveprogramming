...
void solve()
{
	...
	cin >> n;
	vi a(n);
	vvmi dp(n + 1, vmi(n / 2 + 1));
	dp[n][0] = 1;
	for (int i : rep(n)) cin >> a[i];
	for (int r = n - 1; r >= 0; r--) {
		int l = r;
		while (l && a[l - 1] == a[r]) l--;
		for (int j : rep(1, n / 2 + 1)) { // small
			int avail = (max(0, j - 1)) + (j == n / 2);
			int small = n - 1 - r - j;
			int big = j;
			dbg(r, small, big, avail);
			if (small >= 0 && big >= 0 && avail >= small) {
				if (r != l) dp[r][j] += dp[r + 1][j] * C.binom(avail - small, r - l);
			}
		}
		for (int j : rep(1, n / 2 + 1)) { // big
			if (l != r) dp[l][j] += dp[r][j - 1];
			else dp[l][j] += dp[l + 1][j - 1];
		}
		dbg(l, dp[l]);
		for (int j : rep(1, n / 2 + 1)) { // small
			int avail = (max(0, j - 1)) + (j == n / 2);
			...
			dbg(r, small, big, avail);
			if (small >= 0 && big >= 0 && avail >= small) {
				if (l != r) dp[l][j] += dp[r + 1][j] * C.binom(avail - small, r - l + 1);
				else dp[l][j] += dp[r + 1][j] * C.binom(avail - small, r - l + 1);
			}
		}
		dbg(l, dp[l]);
		r = l;
