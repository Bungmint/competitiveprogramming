...
ll dp[1 << 19][19]; // always start with the lowest integer
bool g[19][19];
...
void solve()
{
	...
	for (int mask : rep(1, 1 << n)) {
		...
		for (int la : rep(n)) {
			...
			for (int nxt : rep(n)) {
				if (la != lo && nxt == lo && g[la][lo] && __builtin_popcount(mask) >= 3) {
					ans += dp[mask][la];
					continue;
				}
				if ((mask >> nxt & 1) || nxt < lo || !g[la][nxt]) continue;
