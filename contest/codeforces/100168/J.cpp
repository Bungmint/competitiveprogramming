...
vP line_intersect(const Line& a, const Line& b) {
    ...
    if (sgn(x-y)) return sgn(x) == 0 ? vP{b.fi, b.se} : vP{};
...
void solve() {
	Line l, m;
	cin >> l.fi >> l.se >> m.fi >> m.se;
	cout << fixed << setprecision(15);
	if (sz(seg_intersect(l, m))) cout << T(0) << '\n';
	else{
		ld ret = LINF;
		#define mi(a, b) ckmin(ret, seg_dist(a, b))
		mi(l.fi, m); mi(l.se, m); mi(m.fi, l); mi(m.se, l);
		cout << ret << '\n';
	}
}
