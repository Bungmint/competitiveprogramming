...
void solve() {
    ...
    if (X == N || Y == N) {
        if (X == N) {
            ans = C.binom(Y - 1, N - J) * C.binom(X - Y - 1, J - I - 1); 
        }else{
            ans = C.binom(X - 1, I - 1) * C.binom(Y - X - 1, J - I - 1);
        }
        if (X == N && (I == 1 || I == N)) ans = 0;
        if (Y == N && (J == 1 || J == N)) ans = 0;
    }else{
        for (int ind = 2; ind < I; ind++) {
            if (X < Y) break; 
            ans += C.binom(Y - 1, N - J) * C.binom(X - Y - 1, J - I - 1) * C.binom(N - X - 1, I - ind - 1);
        }
        for (int ind = I + 1; ind < J; ind++) {
            Mint cur = 1;
            if (X < Y) {
                cur *= C.binom(X - 1, I - 1);
                cur *= C.binom(N - Y - 1, J - ind - 1);
                cur *= C.binom(Y - X - 1, N - J - (X - I));
            }else{
                cur *= C.binom(Y - 1, N - J);
                cur *= C.binom(N - X - 1, ind - I - 1);
                cur *= C.binom(X - Y - 1, I - 1 - (Y - 1 - (N - J)));
            }  
            ans += cur;
        }
        for (int ind = J + 1; ind < N; ind++) {
            if (X > Y) break;
            ans += C.binom(X - 1, I - 1) * C.binom(Y - X - 1, J - I - 1) * C.binom(N - Y - 1, ind - J - 1);
        }
    }
    cout << ans << '\n';
}
