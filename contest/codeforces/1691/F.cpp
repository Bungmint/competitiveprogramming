...
void init(int u = 0, int pu = -1) {
    ...
    for (auto &v : g[u]) {
        ...
        init(v, u);
        subtree[u] += subtree[v];
        f[u] -= C.binom(subtree[v], k);
    }
    f[u] += C.binom(subtree[u], k);
    sum += f[u] * subtree[u];
}
void dfs(int u = 0, int pu = -1) {
    ...
    for (auto &v : g[u]) {
        ...
        Mint fu = f[u], fv = f[v];
        Mint cur_sum = sum;
        f[u] += C.binom(subtree[u] - vv, k) + C.binom(vv, k) - C.binom(subtree[u], k);
        f[v] += C.binom(subtree[u], k) - C.binom(subtree[u] - vv, k) - C.binom(vv, k);
        sum -= fu * subtree[u] + fv * subtree[v];
        subtree[u] -= vv;
        subtree[v] = n;
        sum += f[u] * subtree[u] + f[v] * subtree[v];
        dfs(v, u);
        sum = cur_sum;
        subtree[v] = vv;
        subtree[u] = n;
