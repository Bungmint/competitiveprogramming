...
constexpr int M = 1e5 + 10, MOD = 1e9 + 7;
Sieve<M> sieve;
int cnt[M], a[M], dp[M];

int mod_pow(int aa, int b) {
	int r = 1;
	while (b) {
		if (b & 1) r = (1LL*r*aa) % MOD;
		aa = (1LL*aa*aa) % MOD;
		b >>= 1;
	}
	return r;
...
void solve() {
	...
	for (int i = 0; i < n; i++) {
		...
		while (a[i] > 1) {
			...
			while (c--) {
				pp *= p;
				for (int i = 0; i < s; i++) di.pb(di[i]*pp);
			}
		}
		for (auto &e : di) cnt[e]++;
	}
	for (int d = max_a; d >= 1; d--) {
		int& ret = dp[d] = mod_pow(2, cnt[d])-1;
		dbg(ret);
		for (int i = 2*d; i <= max_a; i += d) {
			ret -= dp[i];
			if (ret < 0) ret += MOD;
		}
		if (d == 1) {
