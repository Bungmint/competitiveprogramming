...
void solve() {
	...
	for (int i = 0; i < sz(ind); i++) {
		if (i < sz(ind) - 1) {
			int len = ind[i+1] - ind[i] - 1;
			int lo = lb(all(r), ind[i]+1) - r.begin() - 1;
			int hi = ub(all(l), ind[i+1]-1) - l.begin();
			int cnt = n - (lo+1+n-hi);
			ans += C.binom(cnt, k)*len;
		}
		int len = 1;
		int lo = lb(all(r), ind[i]) - r.begin() - 1;
		int hi = ub(all(l), ind[i]) - l.begin();
		int cnt = n - (lo+1+n-hi);
		ans += C.binom(cnt, k)*len;
	}
	cout << ans << '\n';
