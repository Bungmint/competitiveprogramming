...
Mint count_five(ll n) {
	Mint res = 0;
	for (ll i = 5, p = 1; i <= n; i *= 5, p++) {
		ll last = n / i;
		if (last > 1) {
			res += Mint((last * (last - 1)) / 2) * Mint(i);
		}
		ll L = last * i;
		res += Mint(last)*Mint(n - L + 1);
	}
	return res;
}
...
void solve() {
	int a, b;
	cin >> a >> b;
	if (a <= 1) cout << count_five(b) << '\n';
	else cout << count_five(b) - count_five(a - 1) << '\n';
}
