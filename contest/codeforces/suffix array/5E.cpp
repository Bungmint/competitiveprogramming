...
void solve() {
	...
	SuffixArray sa(s);
	int max_occ{1}, max_len{sz(s)}, id{};
	ll ans{sz(s)};
	vector<vi> ind(sz(s) + 1);
	set<pii> st{{-INF, -INF}, {INF, INF}};
	for (int i = 1; i < sz(s); i++) {
		ind[sa.lcp[i]].pb(i);
	}
	for (int i = sz(s); i >= 0; i--) {
		for (auto &j : ind[i]) {
			auto lt = st.lb(make_pair(j, -INF));
			auto rt = st.lb(make_pair(j, -INF));
			lt = prev(lt);
			auto [ltl, ltr] = *lt;
			auto [rtl, rtr] = *rt;
			int l = j, r = j;
			if (ltr == j - 1) {
				l = ltl;
				st.erase(make_pair(ltl, ltr));
			}
			if (rtl == j + 1) {
				r = rtr;
				st.erase(make_pair(rtl, rtr));
			}
			st.insert(make_pair(l, r));
			ckmax(max_occ, r - l + 2);
			if (ckmax(ans, 1LL*max_occ*i)) {
				max_len = i;
				id = sa.sa[j];
			}
		}
