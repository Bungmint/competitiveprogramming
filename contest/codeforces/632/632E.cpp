...
template<typename T> vector<T> binPow(const vector<T>& A, int k){
	if (k==1) return A;
	vector<T> mid = binPow(A, k>>1);
	F0R(i, sz(mid)) mid[i] = !!((int)mid[i]);
	return (k&1)? mul(mul(mid,mid), A):mul(mid,mid);
}
...
void solve()
{
	...
	vector<Mint> ans = binPow(res, k);
	F0R(i, sz(ans)) if ((int)ans[i])cout << i << " ";
}
