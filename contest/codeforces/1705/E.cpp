...
struct SegTreeLazy{
	...
	T lazy_on_val(T val, U la, int l, int r) {
		if (la == 1) return T(r - l);
        if (la == -1) return T();
        return val;
	}
    int get_rightmost(int v, int pre, int k, int x, int l, int r) {
        push(x, l, r);
        if (r - l == 1) {
            if (t[x].sum == v) return l;
            return l - 1;
        }
        int m = (l + r) >> 1;
        push(2 * x + 1, l, m);
        if (m <= k) {
            return get_rightmost(v, pre - t[2 * x + 1].sum, k, 2 * x + 2, m, r);
        } else if (t[2 * x + 1].sum - pre == v * (m - k)) {
            return get_rightmost(v, 0, m, 2 * x + 2, m, r);
        } else {
            return get_rightmost(v, pre, k, 2 * x + 1, l, m);
        }
    }
    int get_max(int x, int l, int r) {
        push(x, l, r);
        if (r - l == 1) {
            return (t[x].sum == 1 ? l : l - 1);
        }
        int m = (l + r) / 2;
        push(2 * x + 2, m, r);
        return (t[2 * x + 2].sum == 0 ? get_max(2 * x + 1, l, m) : get_max(2 * x + 2, m, r));
    }
};
...
void solve() {
    ...
    cin >> n >> q;
    vi a(n);
    SegTreeLazy<Node, int> seg(MAXN);
    auto add = [&](int k) -> void {
        int mn = seg.get_rightmost(1, seg.query(0, k).sum, k, 0, 0, seg.sz) + 1;
        dbg(k, mn);
        if (k == mn) {
            seg.upd(mn, 1);
        } else {
            seg.range_upd(k, mn, -1);
            seg.upd(mn, 1);
        }
    };
    auto subtract = [&](int k) -> void {
        int mn = seg.get_rightmost(0, seg.query(0, k).sum, k, 0, 0, seg.sz) + 1;
        if (k == mn) {
            seg.upd(mn, 0);
        } else {
            seg.range_upd(k, mn, 1);
