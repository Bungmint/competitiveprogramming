...
void solve()
{
	...
	for (int i=0;i<m;++i){
		...
		// s2 = s+'%'+t;
		// reverse(all(s));
		// SuffixArray sa(s1), revsa(s2);
		// SparseTable<int,17> spar(sa.lcp), revspar(revsa.lcp);
		// int c1 = sa.c[n+1], c2 = revsa.c[n+1];
		// for (int j=n-1;j>=0;--j){
			// int k1 = sa.c[j];
			// pref[j] = spar.query(min(k1,c1), max(k1,c1)-1);
			// int k2 = revsa.c[n-1-j];
			// suf[j] = revspar.query(min(k2,c2), max(k2,c2)-1);
			// ma[j] = max(ma[j+1], suf[j]);
			// dbg(pref[j], suf[j]);
		// }
		// dbg(ans);
		s1 = t + s;
		...
		s2 = t + s;
		reverse(all(s));
		vi p = z_function(s1), q = z_function(s2);
		dbg(p, q);
		for (int j=n-1;j>=0;j--) ma[j] = max(ma[j+1], q[sz(t)+n-1-j]);
		for (int j=0;j<n;++j){
			dbg(p[j+sz(t)], ma[j+sz(t)-1]);
			if (p[j+sz(t)]+ma[j+sz(t)-1]>=sz(t)){ans++;break;}
		}
	}
	cout << ans << endl;
}
