...
void solve() {
	...
	ind.insert(ind.begin(), -INF);
	ind.pb(INF);
	vector<pair<int, Mint>> dp(nind + 1);
	dp[0] = {0, 1};
	// dbg(ind);
	for (int i = 1; i <= nind; i++) {
		...
		for (int j = 0; j < i; j++) {
			if (ind[j] <= ind[i] - nt) {
				auto lt = *lb(all(ind), ind[j] + nt);
				auto rt = *prev(ub(all(ind), ind[i] - nt));
				if (lt > rt) {
					ckmin(minimum, dp[j].fi);
				}
			}
		}
		dp[i].fi = minimum + 1;
		for (int j = 0; j < i; j++) {
			if (ind[j] <= ind[i] - nt && dp[j].fi == minimum) {
				auto lt = *lb(all(ind), ind[j] + nt);
				auto rt = *prev(ub(all(ind), ind[i] - nt));
				if (lt > rt) {
					dp[i].se += dp[j].se;
	...
	for (int i = 0; i <= nind; i++) {
		if (ind[i] + nt > ind[nind]) {
			if (ckmin(minimum, dp[i].fi)) {
				ans = dp[i].se;
			}else if (minimum == dp[i].fi) {
				ans += dp[i].se;
			}
		}
	}
