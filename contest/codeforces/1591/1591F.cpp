...
// URL: https://codeforces.com/contest/1591/problem/F
// Memory Limit: 512 MB
// Time Limit: 3000 ms
// 
...
void solve()
{
	...
	cin >> n;
	vi a(n);
	vector<ar<Mint, 2>> pref(n + 1), dp(n + 1);
	stack<int> st;
	pref[0][0] = dp[0][0] = 1;
	F0R(i, n) {
		cin >> a[i];
		...
		if (sz(st)) {
			int t = st.top();
			F0R(j, 2) {
				dp[i + 1][j] += a[i] * (pref[i][1 - j] - pref[t][1 - j]);
				dp[i + 1][j] += dp[t + 1][j];
			}
		}else{
			F0R(j, 2) {
				dp[i + 1][j] = a[i] * pref[i][1 - j];
			}
		}
		dbg(dp[i + 1]);
		F0R(j, 2) pref[i + 1][j] = pref[i][j] + dp[i + 1][j];
		st.push(i);
	}
	cout << dp[n][n & 1] - dp[n][1 - (n & 1)];
}
