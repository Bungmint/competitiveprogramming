...
using pmi = pair<Mint, Mint998>;
const pmi BASE = {5003, 5003};
pmi power[50000];
...
void precalc() {
    power[0] = {1, 1};
    power[1] = BASE;
    for (int i = 2; i < 50000; i++) power[i] = power[i - 1] * BASE;
}
...
struct HashString {
    ...
    HashString(const string& s) {
        ...
        hashes.resize(n + 1);
        for (int i = 1; i <= n; i++) {
            hashes[i] = hashes[i - 1] * BASE;
            hashes[i] = hashes[i] + pmi({s[i - 1], s[i - 1]});
        }
    }
    pmi get_hash(int l, int r) {
        return hashes[r + 1] - power[r - l + 1] * hashes[l];
    }
};
...
void solve() {
    while (true) {
        ...
        int n = sz(s);
        HashString hsh(s);
        int L = 1, R = n, ans = -1, rig = -1;
        while (L <= R) {
            int mid = (L + R) / 2;
            map<pii, int> freq;
            for (int i = 0; i < n - mid + 1; i++) {
                freq[pii(hsh.get_hash(i, i + mid - 1))]++;
            }
            int rightmost = -1;
            for (int i = 0; i < n - mid + 1; i++) {
                if (freq[pii(hsh.get_hash(i, i + mid - 1))] >= m) {
                    ckmax(rightmost, i);
                }
            }
            if (rightmost == -1) {
