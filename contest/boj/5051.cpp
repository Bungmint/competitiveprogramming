...
vl convolution_mod(const vl &a, const vl &b, const int& M) {
	if (a.empty() || b.empty()) return {};
	vl res(sz(a) + sz(b) - 1);
...
vl convolution_int(const vl& a, const vl& b) {
    ...
    vl ans(sz(res));
    for (int i = 0; i < sz(res); i++) ans[i] = (ll)(res[i] + .5);
    return ans;
}
...
void solve() {
    ...
    vl res = convolution_int(a, a);
    for (int i = 0; i < sz(res); i++) {
        res[i] -= cnt[i];
        ans += res[i] * is_good[i % n] / 2;
    }
    for (int i = 0; i < 2 * n; i++) {
        ans += cnt[i] * is_good[i % n];
    }
    cout << ans << '\n';
}
