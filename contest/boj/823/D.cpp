...
struct DSU {
	...
	vl sum;
	ll ans;
	DSU(int N) { e = vi(N, -1); sum = vl(N, 0); ans = 0; }
...
void solve() {
	...
	for (int i = 0; i < N; i++) {
		cin >> dsu.sum[i];
		ckmax(dsu.ans, dsu.sum[i]);
	}
	for (int i = 0; i < N - 1; i++) {
		int u, v;
		cin >> u >> v, u--, v--;
		edges[i] = {min((int)dsu.sum[u], (int)dsu.sum[v]), u, v};
	}
	sort(all(edges), greater<ar<int, 3>>());
	ll ans{dsu.ans};
	int cur_min{INF};
	for (int i = 0; i < N - 1; i++) {
		auto [w, u, v] = edges[i];
		ckmin(cur_min, w);
		dsu.unite(u, v);
		ckmax(ans, 1LL*cur_min*dsu.size(dsu.get(u)));
	}
	cout << ans << '\n';
}
