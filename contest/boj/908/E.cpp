...
void precalc() {
    int mx = 1;
    for (int i = 0; i < N; i++) mx *= 11;
    pp[0] = 1;
    for (int i = 1; i < N; i++) pp[i] = pp[i - 1] * 11;
    for (int i = 0; i < mx; i++) {
        ar<int, 6> a = to_dig(i);
        for (int j = 0; j < 6; j++) dig[i][j] = a[j];
        for (int j = 0; j < 6; j++) {
            if (dig[i][j]) {
                deg[i]++;
                g[i - pp[j]].pb(i);
...
void solve() {
    cin >> N;
    precalc();
    for (int i = 0; i < N; i++) cin >> M[i] >> p[i], sum += p[i];
