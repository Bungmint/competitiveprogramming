...
// Contest: HackerRank - World CodeSprint 5
// URL: https://www.hackerrank.com/contests/world-codesprint-5/challenges/longest-increasing-subsequence-arrays
// Memory Limit: 512 MB
// Time Limit: 4000 ms
// 
...
void solve()
{
	int m, n;
	cin >> m >> n;
	vector<Mint> powN(m - n + 1, (Mint)1), powN1(m - n + 1, (Mint)1);
	FOR(i, 1, m - n + 1) powN[i] = powN[i - 1] * n, powN1[i] = powN1[i - 1] * (n - 1);
	Mint ans{};
	F0R(x, m - n + 1){
		ans += nCk(m - x - 1, n - 1) * powN[x] * powN1[m - n - x];
	}
	cout << ans << "\n";
}
