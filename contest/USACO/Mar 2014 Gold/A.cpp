...
void solve()
{
	...
	for (int i : rep(n)) {
		int x, y, g;
		cin >> g >> x >> y;
		int xle = (x - k + y), yle = (x - k - y);
		int xri = (x + k + y), yri = (x + k - y);
		ys.pb(yle), ys.pb(yri);
		ys.pb(yle + 1), ys.pb(yri - 1);
		ys.pb(yle - 1), ys.pb(yri + 1);
		a.pb({xle, yle, yri, g});
		a.pb({xri, yle, yri, -g});
	}
	...
	// dbg(ys);
	// dbg(a);
	LazySegTree<Max, ll> st(sz(ys));
	ll ans = 0;
	for (int i = 0; i < sz(a); i++) {
		vector<ar<int, 4>> add, remove;
		int cur = a[i][0];
		while (i < sz(a) && a[i][0] == cur) {
			if (a[i][3] > 0) add.pb(a[i]);
			else remove.pb(a[i]);
			i++;
		}
		i--;
		// dbg(add, remove);
		for (auto &[x, yl, yr, g] : add) {
			yl = lb(all(ys), yl) - ys.begin();
			yr = lb(all(ys), yr) - ys.begin();
			st.rangeUpd(yl, yr + 1, g);
		}
		ckmax(ans, st.query(0, sz(ys)).x);
		for (auto &[x, yl, yr, g] : remove) {
			yl = lb(all(ys), yl) - ys.begin();
			yr = lb(all(ys), yr) - ys.begin();
			st.rangeUpd(yl, yr + 1, g);
		}
