...
void solve()
{
	...
	vector<vector<int>> vis(n, vi(n)), lighton(n, vi(n));
	map<pii, vpi> switches;
	lighton[0][0] = 1;
	REP(m){
		int x, y, a, b;
		re(x, y, a, b);
		x--, y--, a--, b--;
		switches[make_pair(x, y)].pb({a, b});
	}	
	function<void(int, int)> dfs = [&](int r, int c){
		if (r<0||c<0||r>=n||c>=n) return;
		if (vis[r][c]||!lighton[r][c]) return;
		vis[r][c] = 1;
		for (auto&[nr, nc]:switches[make_pair(r, c)]){
			lighton[nr][nc] = 1;
			if (!vis[nr][nc]){
				F0R(k, 4){
					int nnr = nr + dx[k], nnc = nc + dy[k];
					if (nnr<0||nnc<0||nnr>=n||nnc>=n) continue;
					if (vis[nnr][nnc]) dfs(nr, nc);
				}
			}
		}
		F0R(k, 4){
			...
			if (nnr<0||nnc<0||nnr>=n||nnc>=n) continue;
			if (lighton[nnr][nnc]) dfs(nnr, nnc); 
		}
	};
	dfs(0, 0);
	int ans = 0;
	F0R(i, n) ans += count(all(lighton[i]), 1);
	cout << ans;
}
