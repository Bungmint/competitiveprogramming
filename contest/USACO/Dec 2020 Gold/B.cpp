...
// Contest: USACO - USACO 2020 December Contest, Gold
...
void solve()
{
	...
	FOR(i, 1, sz(s)) {
		F0R(mcur, 4) {
			if (s[i] != '?' && BASE[mcur] != s[i]) continue;
			F0R(n, 4) {
				F0R(l, 4) {
					F0R(m, 4) {
						if (m != mcur) dp[i][n][l][mcur] += dp[i - 1][n][l][m];
						if (n == m) dp[i][l][mcur][mcur] += dp[i - 1][n][l][m];
					}
				}
			}
		}
